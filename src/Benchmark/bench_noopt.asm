...
sample_fun_x64 proc
	...
	push		rsi				; benchtask*
	push		rdi				; benchtask->fnptr
	push		r12				; scratch (inner counter - reps, etc.)
	push		r13				; time accumulator
	push		r14				; outer counter - sets
	push		r15				; time at iteration start
	push		rbx				; CPU identifier
...
_sample_fun_x64_inner:
	vxorps		ymm0, ymm0, ymm0	; zero out all register-passed arguments
	...
	rdtscp
	shl			rdx, 32
	mov			rbx, rcx			; save the CPU identifier
	or			rdx, rax
	mov			r15, rdx

	xor			rcx, rcx			; zero out the regs polluted by rdtscp
	xor			rdx, rdx
	call		rdi					; >>> CALL D.U.T.

	rdtscp
	lfence							; make sure rdtscp is not reordered farther than here

	cmp			rcx, rbx
	jne			_sample_fun_x64_inner ; we were moved to anoter CPU, repeat measurement
